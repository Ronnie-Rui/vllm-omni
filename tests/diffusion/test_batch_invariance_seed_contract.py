...
def test_request_gate_rejects_implicit_seed(monkeypatch):
    """The seed validator has to fire before __post_init__ hides the gap.

    Construction assigns a random fallback seed when none was supplied, so the
    contract must reject the request before that fallback runs -- otherwise a
    missing seed would silently look like a valid one.
    """
    monkeypatch.setattr(envs, "VLLM_BATCH_INVARIANT", True)
...
def test_request_gate_rejects_a_fallback_seed_from_a_direct_caller(monkeypatch):
    """seed_was_explicit is the one check params alone cannot make.

    A request built while the switch was off carries the random fallback seed
    __post_init__ assigned, which is indistinguishable from a caller-supplied
    one by looking at sampling_params. Only the recorded explicitness separates
    them, so validating that request afterwards has to reject it.
    """
    monkeypatch.setattr(envs, "VLLM_BATCH_INVARIANT", False)
    request = _request()
...
@pytest.mark.parametrize(("device", "hip"), [(torch.device("cpu"), None), (torch.device("cuda", 0), "6.0")])
def test_worker_bootstrap_skips_unsupported_device_silently(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
    device: torch.device,
    hip: str | None,
) -> None:
    """Non-CUDA and ROCm both skip without a diagnostic, so neither can be inferred."""
    monkeypatch.setattr(envs, "VLLM_BATCH_INVARIANT", True)
    monkeypatch.setattr(torch.version, "hip", hip)
    monkeypatch.setattr(torch.cuda, "get_device_capability", lambda device: pytest.fail())

    with caplog.at_level(logging.DEBUG):
        diffusion_worker_module._initialize_batch_invariance(device)
...
def test_worker_bootstrap_admits_every_cuda_capability(
    ...
    The capabilities here sit below, inside and above the measured 8.9 family, so
    re-adding a bound in either direction fails this test rather than surfacing as
    a startup crash on a user's GPU. Determinism on such a device is scoped by
