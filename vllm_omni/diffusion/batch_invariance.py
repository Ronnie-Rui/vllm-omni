...
def validate_batch_invariant_diffusion_request(request: "OmniDiffusionRequest") -> None:
    ...
    # Not redundant with the seed check above: the request may already carry a
    # seed that __post_init__ assigned as a fallback (request.py:52-53), which
    # params alone cannot distinguish from a caller-supplied one. Vacuous when
    # reached from __post_init__, load-bearing for direct callers.
    require(
