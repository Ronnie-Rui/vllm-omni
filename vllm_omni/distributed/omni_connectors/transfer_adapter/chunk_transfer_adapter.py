...
class OmniChunkTransferAdapter(OmniTransferAdapterBase):
    ...
    def collect_timed_out_request_ids(self, timeout_s: float) -> set[str]:
        ...
        # ``restore_queues()`` re-extends ``running_queue`` from this deque
        # unconditionally, so a timed-out request left here would be re-admitted
        # after the scheduler already failed it. See vllm-project/vllm-omni#5349.
        self._held_non_active = deque(req for req in self._held_non_active if req.request_id not in timed_out_ids)
